...
import { 
  ...
  Share2, ArrowLeft, Clock, DollarSign, Calendar, Wifi, Car, Heart,
...
const TripMap = () => {
  ...
  const { itinerary, destination, formData } = location.state || {};
  ...
  const [mapCenter, setMapCenter] = useState([9.9192, 78.1194]); // Default: Madurai
  ...
  // **FREE: Get coordinates using Nominatim (OpenStreetMap)**
  ...
  // Geocode all activities for the current day
  const geocodeActivities = async () => {
    ...
    try {
      ...
      for (const activity of activities) {
        const coords = await geocodeAddress(activity.address || activity.activity);
        if (coords) {
          coordinates.push({
            ...coords,
            activity: activity,
      ...
      setActivityCoordinates(coordinates);
      
      // Set map center to first activity or average of all activities
  ...
  const calculateRoute = async () => {
    ...
    setLoading(true);
    try {
      const coordsString = activityCoordinates.map(coord => `${coord.lng},${coord.lat}`).join(';');
      const response = await fetch(
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        setRouteData({
          ...
          steps: activityCoordinates.map((coord, index) => ({
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-200/50 p-4 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600 text-sm">Real OpenStreetMap • 100% Free</p>
          </div>
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-2xl p-1 border border-gray-200 shadow-lg h-[500px] mb-6">
              {loading ? (
                ...
                >
                  ...
                  {/* Activity markers */}
                  {activityCoordinates.map((coord, index) => (
                    <Marker
                      key={index}
                      position={[coord.lat, coord.lng]}
                      icon={getMapIcon(coord.category)}
                    >
                      <Popup>
                        <div className="p-2 min-w-48">
                          <h3 className="font-bold text-lg mb-1">{coord.activity.activity.replace('Visit ', '')}</h3>
                          <p className="text-sm text-gray-600 mb-2">{coord.activity.description}</p>
                          <div className="space-y-1 text-xs">
                            <div className="flex justify-between">
                              <span>Cost:</span>
                              <span className="font-bold text-amber-600">${coord.activity.cost}</span>
                            </div>
                            {coord.activity.time_slot && (
                              <div className="flex justify-between">
                                <span>Time:</span>
                                <span>{coord.activity.time_slot.start_time} - {coord.activity.time_slot.end_time}</span>
                              </div>
                            )}
                            <div className="mt-2 p-2 bg-blue-50 rounded border border-blue-200">
                              <span className="font-medium">Address:</span>
                              <p className="text-gray-600">{coord.name}</p>
                            </div>
                          </div>
                        </div>
                      </Popup>
                  ...
                  {routeData && routeData.geometry && (
                    <Polyline
                      positions={activityCoordinates.map(coord => [coord.lat, coord.lng])}
                      color="blue"
                      weight={4}
                      opacity={0.7}
                    />
                ...
                {currentActivities.map((activity, index) => (
                  <div key={index} className="p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200 hover:border-blue-300 transition-all">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-sm font-bold text-amber-700 bg-amber-100 px-2 py-1 rounded">
                        ${activity.cost}
                      </span>
                    </div>
